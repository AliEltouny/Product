...
export function SodaCookbook() {
  ...
  return (
    <div className="relative max-w-[860px] mx-auto">
      ...
      <div className="relative rounded-[2rem] border-4 border-fizzyo-purple/60 bg-gradient-to-br from-[#f8eec8] to-[#f2e3ae] dark:from-[#2b1e23] dark:to-[#191116] shadow-[0_16px_40px_rgba(0,0,0,0.28)] overflow-visible px-3 py-3 md:px-4 md:py-4 aspect-video md:aspect-auto md:h-[360px]">
...
function RecipePage({ recipe, isTurningFace = false, onExpand }: { recipe: Recipe; isTurningFace?: boolean; onExpand?: (recipe: Recipe) => void }) {
  return (
    <article className={`p-3 sm:p-3.5 md:p-4 border-r last:border-r-0 border-black/10 dark:border-white/10 h-auto sm:h-[340px] md:h-[360px] bg-gradient-to-br from-[#f7edc5]/90 to-[#f0e2af]/90 dark:from-[#2a1d23]/80 dark:to-[#1d1519]/80 overflow-y-auto sm:overflow-y-visible flex flex-col ${isTurningFace ? 'h-full' : ''}`}>
      <div className="flex items-start gap-2 sm:gap-2.5 mb-2.5 sm:mb-3">
        <img
          src={recipe.image}
          alt={`${recipe.soda} soda`}
          className="h-12 w-12 sm:h-14 sm:w-14 md:h-16 md:w-16 object-contain rounded-lg bg-black/40 border border-black/10 dark:border-white/10 flex-shrink-0"
        />
        <div className="min-w-0 flex-1">
          <p className="inline-flex rounded-full px-2 py-0.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-black/70 dark:text-white/80 bg-black/10 dark:bg-white/10 mb-1.5">
            {recipe.subtitle}
          </p>
          <h4 className="text-sm sm:text-base md:text-lg font-black leading-tight tracking-tight text-black/90 dark:text-white">
            {recipe.title}
          </h4>
          <p className="text-[10px] sm:text-[11px] md:text-xs text-black/70 dark:text-white/75 mt-0.5 leading-snug">{recipe.summary}</p>
        </div>
      </div>

      <div className="h-[0.5px] bg-black/15 dark:bg-white/15 mb-2.5 sm:mb-3" />

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-2 text-[9px] sm:text-[10px] md:text-xs flex-1 overflow-y-auto">
        <div>
          <h5 className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.15em] mb-1 text-black/80 dark:text-white/80">Ingredients</h5>
          <ul className="space-y-0.5 text-black/80 dark:text-white/80 leading-snug">
        ...
        <div>
          <h5 className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.15em] mb-1 text-black/80 dark:text-white/80">Instructions</h5>
          <ol className="space-y-0.5 text-black/80 dark:text-white/80 leading-snug list-decimal list-inside">
      ...
      {onExpand && (
        <button
          ...
          className="mt-2 sm:hidden w-full py-1.5 bg-white font-bold text-black rounded-lg hover:bg-white/90 transition-colors text-xs"
        >
          View Full Recipe
        </button>
      )}
